...
//use cursive::logger::log::debug;
use crate::traits;
use cursive::view::{Selector, SizeCache, View};
use cursive::Printer;
use cursive::Vec2;
use cursive::With;
use cursive::XY;
...
struct Child {
    view: Box<dyn traits::ViewAndTest>,
...
impl Child {
    ...
    fn as_view(&self) -> &dyn traits::ViewAndTest {
        &*self.view
    }
}
    ...
    for ChildIterator<I>
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.inner.next().map(|child| {
            ...
            let length = min(self.available, *child.size.get(self.orientation));
...
impl LinearLayout {
    ...
    ///
    /// Chainable variant.
    pub fn child<V: traits::IntoBoxedView + 'static>(self, view: V) -> Self {
        self.with(|s| s.add_child(view))
    }

    /// Adds a child to the layout.
    pub fn add_child<V: traits::IntoBoxedView + 'static>(&mut self, view: V) {
    ...
    ///
    /// Panics if `i > self.len()`.
    pub fn insert_child<V: traits::IntoBoxedView + 'static>(
        &mut self,
        i: usize,
        view: V,
    ) {
    ...
    pub fn get_child(&self, i: usize) -> Option<&dyn traits::ViewAndTest> {
    ...
    pub fn get_child_mut(
        &mut self,
        i: usize,
    ) -> Option<&mut dyn traits::ViewAndTest> {
    ...
    pub fn remove_child(
        &mut self,
        i: usize,
    ) -> Option<Box<dyn traits::ViewAndTest>> {
        ...
        match result {
            EventResult::Ignored => match event {
                ...
                Event::Key(Key::Left) => {
                    ...
                    self.children
                        .iter_mut()
                        .map(|child| child.view.on_event(Event::Key(Key::F6)))
                        .for_each(drop);
                    self.children
                        .iter_mut()
                        .map(|child| child.view.get_three());
                    EventResult::Consumed(None)
                }
                Event::Key(Key::Up)
                        ...
                        && self.focus > 0 =>
                {
                    self.children
                        .iter_mut()
                        .map(|child| child.view.on_event(Event::Key(Key::F9)))
                        .for_each(drop);
                ...
                Event::Key(Key::Right) => {
                    self.children
                        .iter_mut()
                        .map(|child| child.view.on_event(Event::Key(Key::F7)))
                        .for_each(drop);
                    EventResult::Consumed(None)
                }
                Event::Key(Key::Down)
                        ...
                        && self.focus + 1 < self.children.len() =>
                {
                    self.children
                        .iter_mut()
                        .map(|child| child.view.on_event(Event::Key(Key::F8)))
                        .for_each(drop);
